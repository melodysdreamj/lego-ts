...
import {CheckSqlite, CheckSqliteSqlite} from "../../backend/sqlite_example/check";

async function main() {
    console.log("start");

    let check = new CheckSqlite();
    ...
    await CheckSqliteSqlite.upsert(check);

    let check2 = await CheckSqliteSqlite.get('example1');
