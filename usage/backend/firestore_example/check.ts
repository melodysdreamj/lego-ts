...
export class CheckExample {
    ...
    static fromDataString(dataString: string): CheckExample {
        const queryParams = Object.fromEntries(new URLSearchParams(atob(dataString)));

        const object = new CheckExample();
    ...
    static fromMap(queryParams: any): CheckExample {
        const object = new CheckExample();
        ...
        object.t000 = queryParams.t000?.toDate() || new Date(0);
        // object.t001 = queryParams.t001?.toDate() || new Date(0);
        // object.t002 = queryParams.t002?.toDate() || new Date(0);
        // object.t003 = queryParams.t003?.toDate() || new Date(0);
        // object.t004 = queryParams.t004?.toDate() || new Date(0);
        // object.t005 = queryParams.t005?.toDate() || new Date(0);
        // object.t006 = queryParams.t006?.toDate() || new Date(0);
        // object.t007 = queryParams.t007?.toDate() || new Date(0);
        // object.t008 = queryParams.t008?.toDate() || new Date(0);
        // object.t009 = queryParams.t009?.toDate() || new Date(0);
        // object.t010 = queryParams.t010?.toDate() || new Date(0);
        // object.t011 = queryParams.t011?.toDate() || new Date(0);
        // object.t012 = queryParams.t012?.toDate() || new Date(0);
        // object.t013 = queryParams.t013?.toDate() || new Date(0);
        // object.t014 = queryParams.t014?.toDate() || new Date(0);
        // object.t015 = queryParams.t015?.toDate() || new Date(0);
        // object.t016 = queryParams.t016?.toDate() || new Date(0);
        // object.t017 = queryParams.t017?.toDate() || new Date(0);
        // object.t018 = queryParams.t018?.toDate() || new Date(0);
        // object.t019 = queryParams.t019?.toDate() || new Date(0);
        // object.t020 = queryParams.t020?.toDate() || new Date(0);
...
export class CheckExampleFirestore {
    static db = admin.firestore();

    static configDocRef(): FirebaseFirestore.CollectionReference {
        return this.db
    ...
    static getObjectReference(): FirebaseFirestore.CollectionReference<CheckExample> {
        return this.configDocRef().withConverter({
            toFirestore: (obj: CheckExample): FirebaseFirestore.DocumentData => obj.toMap(),
            fromFirestore: (
                snapshot: FirebaseFirestore.QueryDocumentSnapshot
            ): CheckExample => CheckExample.fromMap(snapshot.data()),
        });
    }

    static async get(docId: string): Promise<CheckExample | undefined> {
        const snapshot = await this.getObjectReference().doc(docId).get();
        if (snapshot.exists) {
    ...
    static  async upsert(object: CheckExample): Promise<void> {
        await this.getObjectReference().doc(object.docId).set(object, {merge: true});
    }

    static  async insertBulk(objects: CheckExample[]): Promise<void> {
        for (let i = 0; i < objects.length; i += 500) {
            const batch = this.db.batch();
            const end = i + 500;
            for (let j = i; j < end; j++) {
                batch.set(this.getObjectReference().doc(objects[j].docId), objects[j]);
            }
            await batch.commit();
        }
    }
